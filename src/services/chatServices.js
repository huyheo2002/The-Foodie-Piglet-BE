      ...
      if (data.roomCreatorId) {
        ...
        if (isExist === false) {
          ...
          if (respon) {
            ...
            if (findRoom) {
              let checkRoomParticipantIsExits = await checkRoomParticipant(
                findRoom.id,
                data.roomCreatorId
              );
              if (checkRoomParticipantIsExits === false) {
                ...
                resolve({
                  errCode: 2,
                  message: "This user is already present in the room",
                  room: findRoom,
                });
              }
            }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      listUserIds.length > 0 &&
        listUserIds.forEach(async (userId) => {
          await db.RoomParticipant.create({
            userId: userId,
            roomId: data.roomId,
          });
      ...
      if (data.userId == findRoom.roomCreatorId) {
        ...
        if (!getAllName) {
          ...
          resolve({
            errCode: 2,
            message:
              "The group name already exists, please give it another name for easier management",
          });
        }
      }
